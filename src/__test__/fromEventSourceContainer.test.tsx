import { fromEventSourceContainer } from '../fromEventSourceContainer'
...
const storeFactory = (initialProps: P) => (ev: EventSource, props$: Observable<P>) => {
  ...
  return merge(init$, name$, increment$, decrement$).pipe(store(), share())
}

test('fromEventSourceContainer', async () => {
  expect.assertions(5)
  const dispatcher = new Dispatcher()
  const Container = fromEventSourceContainer('Counter', storeFactory)
